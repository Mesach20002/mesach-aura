...
export async function GET(request: Request) {
  ...
  if (shouldUseDevSqliteAuth()) {
    return getSqliteAuthUser(identifier)
  }
    ...
    return NextResponse.json(
      {
        connected: false,
        exists: false,
        error: "DATABASE_URL is not configured for Prisma.",
      },
      { status: 503 }
    )
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      recentReports: user.reports.map((report) => ({
        id: report.id,
...
function getSqliteAuthUser(identifier: string) {
  ...
  return NextResponse.json({
    ...
    reportCount: 0,
    recentReports: [],
    authProviders: user.authAccountCount > 0
      ? [{ providerId: "credential", hasPasswordHash: user.hasPasswordHash > 0 }]
      : [],
    createdAt: formatSqliteDate(user.createdAt),
    updatedAt: formatSqliteDate(user.updatedAt),
  })
}

function formatSqliteDate(value: string | number): string {
